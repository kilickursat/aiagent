...
def calculate_rmr(ucs: float, rqd: float, spacing: float, condition: int, groundwater: int, orientation: int) -> Dict:
    ...
    ucs_rating = 15 if ucs > 250 else 12 if ucs > 100 else 7 if ucs > 50 else 4 if ucs > 25 else 2
    rqd_rating = 20 if rqd > 90 else 17 if rqd > 75 else 13 if rqd > 50 else 8 if rqd > 25 else 3
    spacing_rating = 20 if spacing > 2 else 15 if spacing > 0.6 else 10 if spacing > 0.2 else 8 if spacing > 0.06 else 5
    total_rmr = ucs_rating + rqd_rating + spacing_rating + condition + groundwater + orientation
    rock_class = "I - Very good rock" if total_rmr > 80 else "II - Good rock" if total_rmr > 60 else "III - Fair rock" if total_rmr > 40 else "IV - Poor rock" if total_rmr > 20 else "V - Very poor rock"
    return {
        "rmr_value": total_rmr,
...
def calculate_q_system(rqd: float, jn: float, jr: float, ja: float, jw: float, srf: float) -> Dict:
    ...
    quality = "Exceptionally Good" if q_value > 40 else "Very Good" if q_value > 10 else "Good" if q_value > 4 else "Fair" if q_value > 1 else "Poor" if q_value > 0.1 else "Extremely Poor"
    return {
...
def visualize_3d_results(coordinates: str, geology_data: str, analysis_data: str) -> Dict:
    ...
    stats = {
        "tunnel_length": sum(math.sqrt((x[i]-x[i-1])**2 + (y[i]-y[i-1])**2 + (z[i]-z[i-1])**2) for i in range(1, len(x))),
        "depth_range": [min(z), max(z)],
        "critical_sections": [i for i, r in enumerate(analysis_results['stability']) if r['factor_of_safety'] < 1.5]
    }
...
def predict_cutter_life(ucs: float, penetration: float, rpm: float, diameter: float, cai: float, constants: Dict[str, float]) -> Dict:
    ...
    cl = (constants['C1'] * (ucs ** constants['C2'])) / ((penetration ** constants['C3']) * (rpm ** constants['C4']) * (diameter ** constants['C5']) * (cai ** constants['C6']))
...
if st.session_state.current_analysis:
    ...
    if analysis_type == "Tunnel Support":
        ...
        fig.add_trace(go.Scatter(
            ...
            y=[st.session_state.analysis_params["depth"], st.session_state.analysis_params["depth"]],
            mode='lines',
            name='Tunnel Level'
        ))
        fig.update_layout(
        ...
        if "component_ratings" in st.session_state.current_analysis:
            ...
            fig = go.Figure(data=[
                go.Bar(x=[labels[k] for k in ratings.keys()], y=list(ratings.values()))
            ])
            fig.update_layout(
